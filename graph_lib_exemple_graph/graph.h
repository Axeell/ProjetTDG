...
class Vertex
{
    ...
    private :
       int m_indice;
        ///const d'evolution pour l'equation du modele logistique
        double m_K;
        double m_r;
    ...
    public:
        ...
        Vertex (double value=0, VertexInterface *interface=nullptr) :
            m_K(1.), m_r(),m_img(), m_value(value), m_interface(interface)  {  }

        Vertex (double value, VertexInterface *interface, std::string img, double K, double r, int indice, bool marque = false) :
            m_K(K), m_r(r),m_img(img), m_value(value), m_interface(interface), m_indice(indice), m_marque(marque)   {  }
...
class Edge
{
    ...
    public:
        ...
        Edge (double weight, EdgeInterface *interface, int from, int to) :
            m_weight(weight), m_interface(interface), m_from(from), m_to(to)  {  }
...
class Graph
{
    ...
    public:
        std::map<int, Vertex> m_vertices;
        Vertex get_vertex(int indice);
        /// Les constructeurs sont à compléter selon vos besoin...
        /// Ici on ne donne qu'un seul constructeur qui peut utiliser une interface
        Graph (GraphInterface *interface=nullptr) :
           m_current_time(), m_interface(interface)  {  }

        void add_interfaced_vertex(int idx, double value, int x, int y, double r, double K, std::string pic_name="", int pic_idx=0 );
        void add_interfaced_edge(int idx, int vert1, int vert2, double weight=0);
        /// Méthode spéciale qui construit un graphe arbitraire (démo)
        /// Voir implémentation dans le .cpp
        /// Cette méthode est à enC:\Users\HP\Desktop\graph_lib_exemple_graph\graph.cpplever et remplacer par un système
        /// de chargement de fichiers par exemple.
        void make_example();
        void first_graph();
        void save_first_graph();
        void ajouter_sommet(double r, double K, std::string pic_name);
        void ajouter_arete(int sommet1, int sommet2);
        void composantes_fortement_connexes();
        void supprimer_une_arete(int indice_arete);
        void supprimer_un_sommet(int indice_sommet);
        void evolution_population();

        int * uneComposanteFortementConnexe ();
